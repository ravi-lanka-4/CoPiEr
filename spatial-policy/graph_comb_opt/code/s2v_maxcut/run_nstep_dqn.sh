...
sup_data_dir=$COTRAIN_DATA/"maxcut_test_100_50/gpickle/labeled/"
sup_sol_dir=$(replace $sup_data_dir "gpickle" "sol")
sup_sol_prefix=$COTRAIN_SCRATCH/$sup_sol_dir/"maxcut_test_100_50/"
...
result_root="$COTRAIN_SCRATCH"/$COTRAIN_DATA"/maxcut_test_100_50/"
...
# Data folder
data_dir=$COTRAIN_DATA/"maxcut_test_100_50/gpickle/unlabeled0/"
valid_dir=$COTRAIN_DATA/"maxcut_test_100_50/gpickle/valid/"
modelF=$save_dir/"spatModel.model"
citer=0

## Probability of using a labelelled data point
nlabeled=$(ls -1 $sup_sol_dir/ | wc -l)
nunlabeled=$(ls -1 $data_dir/ | wc -l)
echo "# of labeled: "$nlabeled
echo "# of unlabeled: "$nunlabeled
lambda2=0.25
sup_prob=$( bc -l <<<"($lambda2*$nlabeled)/($nlabeled+$nunlabeled)" )
