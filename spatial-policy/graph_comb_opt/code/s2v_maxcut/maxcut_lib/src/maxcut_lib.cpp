...
int InsertGraph(bool isTest, const int g_id, const int num_nodes, const int num_edges, \
                const int* edges_from, const int* edges_to, const double* weights)
{
...
int InsertGraph(bool isTest, const int g_id, const int num_nodes, \
...
{
    /* Optimal solution available or not */
    if (num_opt != 0){
        auto g1 = std::make_shared<Graph>(num_nodes, num_edges, edges_from, edges_to, \
                                            weights, num_opt, opt_nodes_v1, opt_nodes_v2, num_opt_nodes, prob);
        if (isTest)
            GSetTest.InsertGraph(g_id, g1);
        else
            GSetTrain.InsertGraph(g_id, g1);
    }
    else{
        auto g = std::make_shared<Graph>(num_nodes, num_edges, edges_from, edges_to, weights);
        if (isTest)
...
int PlayGame(const int n_traj, const double eps, bool expert)
{
    Simulator::run_simulator(n_traj, eps, expert);
    return 0;
}
