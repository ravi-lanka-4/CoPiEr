...
eval_spatial(){
  ...
  citer=$( bc -l <<<"$iter*$cmax_iter" )
  modelF=$policyFolder/"spatModel_best"$((citer+SUP_INIT))".model"
...
for (( i=$init; i<$iter; i=$i+1)); do
  ...
  eval_spatial $data "gpickle" ".gpickle" $gexp $i $time_stamp $SUP_INIT $SPAT_ITER
done
