...
train_scip(){
  ...
  scripts/train_bb_search.sh -d $cdata -p 1 -n $n -x .lp -l 10 -u 11 -i 1 \
       -e $experiment -b sel_only_norm.dat
  cd -
}
...
test_scip(){
  ...
  cd $SCIP_HOME
  scripts/test_bb_search.sh -d $cdata -e $experiment -x .lp -o $outF \
      -s $policyDir/searchPolicy.$n.h5 -b $policyDir/searchPolicy.$n.norm.dat
  cd -
}
...
complete_solution(){
  ...
  bin/scipdagger -r 1 -s scip.set --useSols -o $optSol -f $inpF --sol $outSol \
                 --nodepru oracle --nodesel policy $policyDir\searchPolicy.$num.h5 > $logF
}
