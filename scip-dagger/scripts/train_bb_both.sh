...
while getopts ":hd:p:n:c:e:w:tx:m:r:l:u:i:j:k:f:a:b:s:q:w" arg; do
  case $arg in
    h)
      usage
      exit 0
...
for (( j=$low; j<$high; j=$j+$increment )); do
  ...
  for i in `seq 1 $numPasses`; do
      ...
      for prob in `ls $curDatDir`; do
        ...
        if [ -z $searchPolicy ]; then
          ...
          if [ "$datDir" != "$prevDatDir" ]; then
            ...
            for ((kk=1; kk<=$loop; kk++)); do
              echo bin/scipdagger -r $freq -s scip_silent.set -f $prob --nodesel policy $prevSearchPolicy -n 10000 \
                                  --nodepru policy $prevKillPolicy --allsols_write $curPolicySol/$base/$i/ --sol $curPolicySol/$base.sol.$kk \
                                  -j $pru_noise_iter -k $sel_noise_iter --probfeats "$probfeatsIter" --pnorm "$prevPruNormFile" --snorm "$prevSelNormFile"
              bin/scipdagger -r $freq -s scip_silent.set -f $prob --nodesel policy $prevSearchPolicy -n 15000 \
                                  --nodepru policy $prevKillPolicy --allsols_write $curPolicySol/$base/$i/ --sol $curPolicySol/$base.sol.$kk \
                                  -j $pru_noise_iter -k $sel_noise_iter --probfeats "$probfeatsIter" --pnorm "$prevPruNormFile" --snorm "$prevSelNormFile"
            done
            ...
            echo oracle
            echo bin/scipdagger -r $freq -s scip.set -f $prob -o $curPolicySol/$base.sol --nodesel oracle --nodeseltrj $searchTrjIter \
                                --probfeats "$probfeatsIter" --nodepru oracle --nodeprutrj $killTrjIter 
            bin/scipdagger -r $freq -s scip.set -f $prob -o $curPolicySol/$base.sol --nodesel oracle --nodeseltrj $searchTrjIter \
                                --probfeats "$probfeatsIter" --nodepru oracle --nodeprutrj $killTrjIter --pscale 2 --sscale 2 | tee $tmpLog
          ...
          # Produce the solution file in Policy mode 
          echo bin/scipdagger -r $freq -s scip.set -f $prob --nodesel policy $searchPolicy \
          ...
          bin/scipdagger -r $freq -s scip.set -f $prob --nodesel policy $searchPolicy \
                              --nodepru policy $killPolicy --allsols_write $curPolicySol/$base/$i/ \
  	  ...
  	  echo "Primal Bound: "$pb
          ...
          echo bin/scipdagger -r $freq -s scip.set -f $prob \
                            ...
                            --allsols_prefix $curPolicySol/$base/$i/ -o $opt_sol -j $pru_noise_iter -k $sel_noise_iter \
                            --probfeats "$probfeatsIter" --pnorm "$pruNormFile" --snorm "$selNormFile"
          bin/scipdagger -r $freq -s scip.set -f $prob \
                            --nodesel dagger $searchPolicy --nodeseltrj $searchTrjIter \
                            --nodepru dagger $killPolicy --nodeprutrj $killTrjIter \
                            --allsols_prefix $curPolicySol/$base/$i/ -o $opt_sol \
                            --probfeats "$probfeatsIter" --pnorm "$pruNormFile" --snorm "$selNormFile" | tee $tmpLog
    ...
    if ! [ $neural_nets = true ]; then
      ...
      if [ $sel_norm = true ]; then
        selNormFile=$policyDir/searchPolicy.$numPolicy.norm.dat
        echo python pyscripts/normdata.py --i $searchTrjTrain --o $searchTrjTrain.norm --n $selNormFile --f $numfeats
        echo python pyscripts/normdata.py --i $searchTrjValid --o $searchTrjValid.norm --p $selNormFile --f $numfeats
        #echo python pyscripts/normdata.py --i $searchTrjTest --o $searchTrjTest.norm --p $selNormFile --f $numfeats
      ...
      if [ $pru_norm = true ]; then
        pruNormFile=$policyDir/killPolicy.$numPolicy.norm.dat
        echo python pyscripts/normdata.py --i $killTrjTrain --o $killTrjTrain.norm --n $pruNormFile --f $numfeats
        echo python pyscripts/normdata.py --i $killTrjValid --o $killTrjValid.norm --p $pruNormFile --f $numfeats
        #echo python pyscripts/normdata.py --i $killTrjTest --o $killTrjTest.norm --p $pruNormFile --f $numfeats
        echo python pyscripts/prunenet.py --v $killTrjValid.norm --t $killTrjTrain.norm --m $killPolicy --w $killTrjTrain.weight --x $killTrjValid.weight
        python pyscripts/normdata.py --i $killTrjTrain --o $killTrjTrain.norm --n $pruNormFile --f $numfeats
        ...
        python pyscripts/prunenet.py --v $killTrjValid.norm --t $killTrjTrain.norm --m $killPolicy --w $killTrjTrain.weight --x $killTrjValid.weight
        python pyscripts/feat_var.py --i $killTrjTrain.norm --m $scratch/$datDir/$experiment/mean.$numPolicy.txt --v $scratch/$datDir/$experiment/variance.$numPolicy.txt
      else
        python pyscripts/prunenet.py --v $killTrjValid --t $killTrjTrain --m $killPolicy --w $killTrjTrain.weight --x $killTrjValid.weight
        python pyscripts/feat_var.py --i $killTrjTrain --m $scratch/$datDir/$experiment/mean.$numPolicy.txt --v $scratch/$datDir/$experiment/variance.$numPolicy.txt
      fi
    fi
