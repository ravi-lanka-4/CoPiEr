...
usage() {
  echo "Usage: $0 -d <data_path_under_dat> -o <training_data_path_policy> -e <experiment> -n <policy_id> -x <suffix> -m <neural_nets_bool> -f <prob_feats_dir> -a <prune_norm_bool>"
...
pru_norm=false
old_data=""
while getopts ":hd:o:e:x:c:w:n:m:f:a:" arg; do
  case $arg in
    h)
      usage
      exit 0
...
if ! [ $neural_nets = true ]; then
  ./scripts/test_bb_pru_only.sh -d $data -e $experiment/policy$n -x $suffix -k $policyDir/killPolicy.$n \
                       -f "$prob_feats" -a "$pru_norm_file"
else
  ./scripts/test_bb_pru_only.sh -d $data -e $experiment/policy$n -x $suffix -k $policyDir/killPolicy.$n.h5 \
                       -f "$prob_feats" -a "$pru_norm_file"
fi

# get statistics from each log file
echo "getting stats for $data with $experiment c=$c w=$w n=$n"
stats=$(./scripts/get_stats.sh -d $data -e $experiment/policy$n/ -n $n -x $suffix | tail -n 1 | cut -d' ' -f3)
...
./scripts/test_scip.sh -d $data -x $suffix -e scip/total-node-limit/$experiment/policy$n -n $nnodes
...
./scripts/test_gurobi.sh -d $data -n $nnodes -e gurobi/node-limit/$experiment/policy$n -x $suffix -p 1
