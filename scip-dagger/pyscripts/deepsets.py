...
def getRankModel(inp_dim, l1=64, l2=64, l3=64, latent_dim=8):
  ...
  ada = keras.optimizers.Adagrad(lr=0.01, epsilon=None, decay=10**-5)
  model.compile(optimizer=ada, loss = "binary_crossentropy", \
                 metrics=['accuracy'])
  outnet = Model(ranknet.layers[0].input, ranknet.layers[-2].output)

  modelSave = Model(inputs=node1, outputs=node1score)
  return model, modelSave, outnet
...
def main():
  ...
  for k, cidx in enumerate(group):
    ...
    # Save mode
    jsonfname = modelF.replace('.h5', '.json')
    getScore.save(jsonfname)
